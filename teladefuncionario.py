...
def criar_botao_pill(janela, texto, comando=None, icone=None):
    ...
    def on_enter(event): desenhar(cor_hover)
    def on_leave(event): desenhar(cor_normal)
    def on_click(event): 
        if comando:
            comando()
...
def tela_login():
    ...
    # =====================================
    #  NOVO BOTÃO: VISUALIZAR RELATÓRIO
    # =====================================
    def ver_relatorio():
        messagebox.showinfo("Relatório", "Abrindo relatório...")

    botao_relatorio = criar_botao_pill(
        janela,
        "Relatório",
        comando=ver_relatorio,
        icone="📊"
    )
    botao_relatorio.pack(pady=15)

    # BOTÃO CADASTRAR (mesmo lugar)
    ...
    # =====================================
    #   BOTÃO VOLTAR (NOVO, EMBAIXO)
    # =====================================
    botao_voltar = criar_botao_pill(
        janela,
        "Voltar",
        comando=lambda: janela.destroy(),
        icone="↩️"
    )
