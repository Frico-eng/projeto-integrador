...
frame_esq = ctk.CTkFrame(app, width=220)
...
# cria um sub-frame para alinhar imagem à esquerda e textos à direita
...
# frame de textos (à direita da imagem)
...
label_genero2 = ctk.CTkLabel(frame_textos, text="Direção", font=("Arial", 14, "bold"))
label_genero2.pack(anchor="nw")
...
# ----- frame para sessões (embaixo de tudo) -----
frame_sessoes = ctk.CTkFrame(frame_dir, height=150)
...
def mostrar_sessoes(filme):
    for widget in frame_sessoes.winfo_children():
        widget.destroy()
    linha = 0
    linha = criar_linha_sessoes(frame_sessoes, "Dublado:", filme.get("sessoes_dublado", []), linha)
    linha = criar_linha_sessoes(frame_sessoes, "Legendado:", filme.get("sessoes_legendado", []), linha)
...
# cria botões (um por filme)
for idx, filme in enumerate(filmes):
    btn = ctk.CTkButton(scroll, text=filme["titulo"], width=180, height=36, command=lambda i=idx: mostrar_filme(i))
    btn.pack(pady=4, padx=6, fill="x")

# seleciona o primeiro filme
if filmes:
    mostrar_filme(0)
